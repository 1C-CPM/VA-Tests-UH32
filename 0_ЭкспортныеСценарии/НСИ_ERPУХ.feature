...
Сценарий: Для партнера 'ТекИмяПартнера' контрагента 'ТекИмяКонтрагента' я создаю договор с видом 'ТекВидДоговора' номер 'ТекНомер' от 'ТекДата' организация 'ТекОрганизация' ВалютаВзаиморасчетов 'ТекВалюта' ВалютаПлатежа 'ТекВалютаПлатежей' сумма 'ТекСумма' соглашение 'ТекСоглашение' в 1C:ERPУХ
	...
	* Удаляем договор
		...
		Если открылось окно 'Проверка контрагентов' Тогда
			И я нажимаю на кнопку с именем 'РучнаяПроверка'
		И Я в списке "* (Контрагент *)" по полю "Рабочее наименование" ищу элемент '[ТекНомер] от [ТекДата]' "По точному совпадению"
		И Пока в таблице 'Список' количество строк 'больше' 0 Тогда
			И Я удаляю текущую строку в списке 'Список'
			И Я запоминаю значение выражения 'СтрЗаменить(Новый УникальныйИдентификатор, "-", "")' в переменную 'УИД'
			И в таблице 'Список' я выбираю текущую строку
	...
	* Создаем договор
		...
		И из выпадающего списка с именем 'Организация' я выбираю по строке '[ТекОрганизация]'
		И из выпадающего списка с именем 'ВалютаВзаиморасчетов' я выбираю точное значение '[ТекВалюта]'
		И из выпадающего списка с именем 'ОсновнаяВалютаПлатежей' я выбираю точное значение '[ТекВалютаПлатежей]'
		И я устанавливаю флаг с именем 'ФиксированнаяСуммаДоговора'
		И в поле с именем 'СуммаДоговора' я ввожу текст '[ТекСумма]'		
		Тогда открылось окно "Договор с * (создание) *"
		И я нажимаю на кнопку с именем 'ФормаПровестиИЗакрыть'
	...
	* Закрываем форму
		...
		Когда открылось окно "* (Партнер)"
		И Я закрываю окно "* (Партнер)"
		Тогда открылось окно "Партнеры"
		И Я закрываю окно "Партнеры"
...
Сценарий: Я создаю группы финансового учета номенклатуры с именем 'TheName' в 1C:ERPУХ

	* Открываем список
		И я открываю основную форму списка справочника 'ГруппыФинансовогоУчетаНоменклатуры'
		Тогда открылось окно 'Группы настроек финансового учета номенклатуры'			

	* Удаляем элемент
		И Я в списке "Группы настроек финансового учета номенклатуры" по полю "Наименование" ищу элемент '[TheName]' "По началу строки"		
		И Пока в таблице 'Список' количество строк 'больше' 0 Тогда
			И в таблице 'Список' я выбираю текущую строку
			И Я удаляю текущий элемент в окне "[TheName] (Группа настроек финансового учета номенклатуры )" очищая

	* Создаем элементм
		И я нажимаю на кнопку с именем 'ФормаСоздать'
		И в поле с именем 'Наименование' я ввожу текст '[TheName]'
		И я нажимаю на кнопку с именем 'ФормаЗаписатьИЗакрыть'
		И я жду закрытия окна "Группа настроек финансового учета номенклатуры (создание) *" в течение 20 секунд	

	* Закрываем список
			И Я закрываю окно 'Группы настроек финансового учета номенклатуры'

Сценарий: Я создаю вид цены с именем 'TheName' валютой 'TheCurrency' в 1C:ERPУХ
	...
	* Удаляем элемент
		И Я в списке "Виды цен*" по полю "Наименование" ищу элемент '[TheName]' "По началу строки"		
		И Пока в таблице 'Список' количество строк 'больше' 0 Тогда
			И в таблице 'Список' я выбираю текущую строку
			И Я удаляю текущий элемент в окне "* (Вид цены)" очищая
