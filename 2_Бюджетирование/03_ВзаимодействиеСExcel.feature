...
Сценарий: 03.02 Создание внешней информационной базы "ВА - Импорт из Excel"
	...
	И Я создаю внешнюю информационную базу "ВА - Импорт из Excel" тип "Файлы Microsoft Excel" каталог "$КаталогПроекта$\2_Бюджетирование\Макеты\03" тип хранилища ''

	* Тестируем механизм маски имени
		Тогда элемент формы с именем 'МаскаПолногоИмени' стал равен "$КаталогПроекта$\2_Бюджетирование\Макеты\03\*.xls?"
		...
		Тогда таблица 'СтруктураИмени' стала равной:
			...
			| '\\'           | "Проект"            |
			| '\\'           | "Вид отчета"        |
		Тогда элемент формы с именем 'МаскаПолногоИмени' стал равен "$КаталогПроекта$\2_Бюджетирование\Макеты\03\<Период сценария>\<Сценарий>\<Организация>\<Проект>\<Вид отчета>.xls?"
		Когда открылось окно '$ЗаголовокОкна$'
		...
		Тогда элемент формы с именем 'МаскаПолногоИмени' стал равен "$КаталогПроекта$\2_Бюджетирование\Макеты\03\<Вид отчета>+<Организация>.xls?"
...
Сценарий: 03.03 Создание многопериодного бланка отчета и выгузка в Excel
	...
	* Сохраняем бланк
		...
		И в поле с именем 'ИмяФайлаВыгрузки' я ввожу текст "$КаталогПроекта$\2_Бюджетирование\Макеты\03\ВА_ИмпортExcel+Шаблон.xlsx"
	...
	* Сравниваем файл
		...
		И в поле с именем 'ПутьКДокументу1' я ввожу текст "$КаталогПроекта$\2_Бюджетирование\Макеты\03\ВА_ИмпортExcel+Шаблон.xlsx"
		И в поле с именем 'ПутьКДокументу2' я ввожу текст "$КаталогПроекта$\2_Бюджетирование\Макеты\03\ВА_ИмпортExcel_Шаблон.mxl"
...
Сценарий: 03.07 Создание экземпляра отчета - "ВА - ИмпортExcel"	и загрузка из файла
	...
	* Документ должен быть пуст
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету "2_Бюджетирование\Макеты\03\ВА_ИмпортExcel_Пустой.mxl"
	...
	* Загружаем ошибочный файл
		...
		И в поле с именем 'ПутьКФайлу' я ввожу текст "$КаталогПроекта$\2_Бюджетирование\Макеты\03\ВА_ИмпортExcel+Ошибка.xlsx"
		И я нажимаю на кнопку с именем 'ФормаВыбрать'		
		...
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету "$КаталогПроекта$\2_Бюджетирование\Макеты\03\ВА_ИмпортExcel_Пустой.mxl"
	...
	* Загружаем с файлом
		...
		И я меняю значение переключателя с именем 'СпособИмпортирования' на "Файл на диске"
		Тогда у элемента формы с именем 'ПутьКФайлу' текст редактирования стал равен "$КаталогПроекта$\2_Бюджетирование\Макеты\03\ВА_ИмпортExcel+Ошибка.xlsx"
		И в поле с именем 'ПутьКФайлу' я ввожу текст "$КаталогПроекта$\2_Бюджетирование\Макеты\03\ВА_ИмпортExcel+Меркурий.xlsx"
		И я нажимаю на кнопку с именем 'ФормаВыбрать'
		...
		Тогда открылось окно '$ЗаголовокОкна$'
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету "$КаталогПроекта$\2_Бюджетирование\Макеты\03\ВА_ИмпортExcel_Меркурий.mxl"
...
Сценарий: 03.08 Повторная загрузка данных из файла
	...
	* Вибираем второй файл
		...
		Когда открылось окно "Выбор способа расчета показателей"
		И я меняю значение переключателя с именем 'СпособИмпортирования' на "Файл на диске"
		И в поле с именем 'ПутьКФайлу' я ввожу текст "$КаталогПроекта$\2_Бюджетирование\Макеты\03\ВА_ИмпортExcel+Меркурий_2.xlsm"
		И я нажимаю на кнопку с именем 'ФормаВыбрать'
		Тогда открылось окно '$ЗаголовокОкна$'
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету "$КаталогПроекта$\2_Бюджетирование\Макеты\03\ВА_ИмпортExcel_Меркурий_2.mxl"
...
Сценарий: 03.09 Создаем сводную таблицу
	...
	* Сверяем таблицу
		Когда открылось окно '$ЗаголовокОкна$'
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету "$КаталогПроекта$\2_Бюджетирование\Макеты\03\ВА_ИмпортExcel_Меркурий_2_СТ.mxl"
	...
	* Загружаем по регламенту
		...
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету "$КаталогПроекта$\2_Бюджетирование\Макеты\03\ВА_ИмпортExcel_Меркурий_СТ.mxl"	
