...
Сценарий: 19.04 Создание внешней информационной базы "ВА - ПроцессИмпортЦена"
	...
	И Я создаю внешнюю информационную базу "ВА - ПроцессИмпортЦена" тип "Файлы Microsoft Excel" каталог "$КаталогПроекта$\2_Бюджетирование\Макеты\19" тип хранилища ''
	...
	* Тестируем механизм маски имени
		...
		Тогда элемент формы с именем 'МаскаПолногоИмени' стал равен "$КаталогПроекта$\2_Бюджетирование\Макеты\19\<Вид отчета>+<Организация>.xls?"
	...
	* Создаем новый документ управления отчетным периодом
		...
		* Кнопки открытия обработок управления процессом
			...
			* Матрица отчетов
				...
				Тогда табличный документ 'Результат' равен макету "2_Бюджетирование\Макеты\19\ВА_ПроцессПодготовкиОтчетности_МатрицаОтчетов1.mxl"	
