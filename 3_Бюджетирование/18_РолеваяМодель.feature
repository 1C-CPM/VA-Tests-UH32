...
Структура сценария: 18.08 Создание экземпляров отчетов
	...
	Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету '3_Бюджетирование\Макеты\18\ВА_ПроверкаНастроекДоступа_20.mxl'
	И Я ввожу значение '100' в ячейку 'R6C2'	
	Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету '3_Бюджетирование\Макеты\18\ВА_ПроверкаНастроекДоступа_100.mxl'
...
Сценарий: 18.12 Проверка экземпляров отчетов для "Бюджетирование1"
	...
	* Ищем экземпляры отчетов	
		...
		Тогда таблица 'Список' содержит "3" строк из списка:
			| "Вид отчета"                              | "Организация"  | "Период отчета"  | "Период окончания" | "Сценарий"               | "Статус"    | "Проект"               | "Аналитика 1"   | "Аналитика 2" | "Аналитика 3" | "Аналитика 4" | "Аналитика 5" | "Аналитика 6" | "Основная валюта" | "Внешняя ИБ" |
	...
	* Открываем документ с доступом на запись
		И в таблице 'Список' я перехожу к строке:
			| "Вид отчета"                              | "Организация"  | "Период отчета"  | "Период окончания" | "Сценарий"               | "Статус"    | "Проект"               | "Аналитика 1"   | "Основная валюта" |
			| "ВА - Проверка настроек доступа (запись)" | "Меркурий ООО" | "Январь 2024 г." | "Март 2024 г."     | "ВА - Основной сценарий" | "Черновик"  | "ВА - Основной проект" | "Администрация" | "RUB"             |
		И в таблице 'Список' я выбираю текущую строку
...
Сценарий: 18.16 Проверка СТ для пользователя "Бюджетирование2"
	...
	* Проверяем сводную таблицу с правами на запись
		...
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету '3_Бюджетирование\Макеты\18\ВА_ПроверкаНастроекДоступа_СТ_100.mxl'
		...
		И Я закрываю окно "Управление версиями показателей *"
		Тогда открылось окно '$ЗаголовокОкна$'
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету '3_Бюджетирование\Макеты\18\ВА_ПроверкаНастроекДоступа_СТ_20.mxl'
	...
	* Проверяем сводную таблицу без прав
		...
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету '3_Бюджетирование\Макеты\18\ВА_ПроверкаНастроекДоступа_НетДоступа.mxl'	
...
Сценарий: 18.19 Проверка СТ для пользователя "Бюджетирование3"
	...
	* Проверяем сводную таблицу с правами на чтение
		...
		И я нажимаю на кнопку с именем 'ОбновитьДанныеТаблицы1'
		И я жду открытия формы '$ЗаголовокОкна$' в течение 20 секунд
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету '3_Бюджетирование\Макеты\18\ВА_ПроверкаНастроекДоступа_СТ_Пустой.mxl'
		И Я ввожу значение '100' в ячейку 'R4C2'
		Дано Табличный документ 'ПолеТабличногоДокументаМакет' равен макету '3_Бюджетирование\Макеты\18\ВА_ПроверкаНастроекДоступа_СТ_Пустой.mxl'
...
Сценарий: 18.23 Тест интерфейса для пользователя "Бюджетирование1" - Виды отчетов, экземпляры отчетов, сводная таблица
	...
	* Список экземпляров отчетов
		...
		Тогда таблица 'Список' стала равной:
			| "Вид отчета"                              | "Организация"  | "Период отчета"  | "Период окончания" | "Сценарий"               | "Статус"    | "Проект"               | "Исполнитель" | "Согласование" | "Аналитика 1"   | "Аналитика 2" | "Аналитика 3" | "Аналитика 4" | "Аналитика 5" | "Аналитика 6" | "Основная валюта" | "Внешняя ИБ" |
			| "ВА - Проверка настроек доступа (запись)" | "Меркурий ООО" | "Январь 2024 г." | "Март 2024 г."     | "ВА - Основной сценарий" | "Черновик"  | "ВА - Основной проект" | ""            | ""             | "Администрация" | ""            | ""            | ""            | ""            | ""            | "RUB"             | ""           |
...
Сценарий: 18.29 Тест интерфейса для пользователя "Бюджетирование1" - Организации
	...
	ИначеЕсли '$$ЭтоУХ$$' Тогда
		...
		Тогда открылось окно "Реквизиты Меркурий ООО"
		И Я закрываю окно "Реквизиты Меркурий ООО"
...
Сценарий: 18.34 Тест интерфейса для пользователя "Бюджетирование1" - Матрица экземпляров отчетов
	...
	* Сверяем результат
		Тогда табличный документ 'Результат' равен:
			...
			| ''                                                                                              | ''                     | ''                                                             | ''           | ''          | ''         | ''             | ''            |
			| ''                                                                                              | ''                     | ''                                                             | ''           | ''          | ''         | ''             | ''            |
			| ''                                                                                              | ''                     | " - Экземпляр отчета со статусом \"Не заполнен\""              | ''           | ''          | ''         | ''             | ''            |
			| ''                                                                                              | ''                     | " - Экземпляр отчета со статусом \"Черновик\""                 | ''           | ''          | ''         | ''             | ''            |
			| ''                                                                                              | ''                     | " - Экземпляр отчета со статусом \"Проверен, на утверждении\"" | ''           | ''          | ''         | ''             | ''            |
			| ''                                                                                              | ''                     | " - Экземпляр отчета со статусом \"Отправлен на уточнение\""   | ''           | ''          | ''         | ''             | ''            |
			| ''                                                                                              | ''                     | " - Экземпляр отчета со статусом \"Утвержден\""                | ''           | ''          | ''         | ''             | ''            |
			| ''                                                                                              | ''                     | " - Экземпляр отчета со статусом \"Заполнен с ошибками\""      | ''           | ''          | ''         | ''             | ''            |
