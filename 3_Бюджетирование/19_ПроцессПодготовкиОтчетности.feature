...
Сценарий: 19.04 Создание внешней информационной базы "ВА - ПроцессИмпортЦена"
	...
	И Я создаю внешнюю информационную базу "ВА - ПроцессИмпортЦена" тип "Файлы Microsoft Excel" каталог "$КаталогПроекта$\3_Бюджетирование\Макеты\19" тип хранилища ''
	...
	* Тестируем механизм маски имени
		...
		Тогда элемент формы с именем 'МаскаПолногоИмени' стал равен "$КаталогПроекта$\3_Бюджетирование\Макеты\19\<Вид отчета>+<Организация>.xls?"
...
Сценарий: 19.09 Настройка процесса подготовки отчетности
	...
	* Этап для "ВА - ПроцессКоличество"
		...
		* Организации
			...
			И в таблице 'ОрганизационныеЕдиницыОтборСКДНастройкиОтбор' из выпадающего списка с именем 'ОрганизационныеЕдиницыОтборСКДНастройкиОтборПравоеЗначение' я выбираю по строке 'Меркурий ООО'
			И в таблице 'ОрганизационныеЕдиницыОтборСКДНастройкиОтбор' из выпадающего списка с именем 'ОрганизационныеЕдиницыОтборСКДНастройкиОтборПравоеЗначение' я выбираю по строке 'Венера ООО'
			Тогда открылось окно "Этап процесса подготовки отчетности (Этап универсального процесса) *"
	...
	* Создаем новый документ управления отчетным периодом
		...
		* Кнопки открытия обработок управления процессом
			...
			* Матрица отчетов
				...
				Тогда табличный документ 'Результат' равен макету "3_Бюджетирование\Макеты\19\ВА_ПроцессПодготовкиОтчетности_МатрицаОтчетов1.mxl"	
...
Сценарий: 19.12 Ручное заполнение - "ВА - ПроцессКоличество"
	...
	* Отменяем утверждение
		...
		И в таблице 'ТаблицаОрганизаций' я нажимаю на кнопку с именем 'УстановитьФлажки'				
...
Сценарий: 19.14 Импорт - "ВА - ПроцессЦена"	
	...
	* Заполняем из формы управления процессом по организации Венера
		...
		И я выбираю пункт контекстного меню с именем 'ДеревоЭтапыКонтекстноеМенюЗавершитьОбработкуЭтапа' на элементе формы с именем 'ДеревоЭтапы'
		Тогда открылось окно "Завершить обработку этапа для организаций:"
		И в таблице 'ТаблицаОрганизаций' я нажимаю на кнопку с именем 'СнятьФлажки'
		И в таблице 'ТаблицаОрганизаций' я перехожу к строке:
			| "Выбран" | "Организация" |
			| "Нет"    | "Венера ООО"  |
		И в таблице 'ТаблицаОрганизаций' я устанавливаю флаг с именем 'ТаблицаОрганизацийВыбран'
		И в таблице 'ТаблицаОрганизаций' я завершаю редактирование строки				
		И я нажимаю на кнопку с именем 'Выполнить'
		Тогда таблица 'ДеревоОперации' содержит '3' строк из списка:
			| "Шаблон документа / Организация" | "Состояние" |
...
Сценарий: 19.15 Консолидация - "ВА - ПроцессСумма"
	...
	* Сверяем итоговый отчет
		...
		Тогда табличный документ 'ПолеТабличногоДокументаМакет' равен:
			...
			| "Товары"                                                          | '90'             | '6 030 000' | '115 600 000' | '180'             | '6 235 000' | '238 400 000'| '270'          | '6 440 000' | '368 400 000' | '540'        | '18 705 000' | '722 400 000' |
			| "1С:ERP. Управление холдингом "                                   | '10'             | '1 950 000' | '19 500 000'  | '20'              | '2 000 000' | '40 000 000' | '30'           | '2 050 000' | '61 500 000'  | '60'         | '6 000 000'  | '121 000 000' |
			| "1С:Корпорация "                                                  | '30'             | '2 050 000' | '61 500 000'  | '60'              | '2 100 000' | '126 000 000'| '90'           | '2 150 000' | '193 500 000' | '180'        | '6 300 000'  | '381 000 000' |
			...
			| "1С:Управление холдингом 8 "                                      | '20'             | '1 250 000' | '25 000 000'  | '40'              | '1 300 000' | '52 000 000' | '60'           | '1 350 000' | '81 000 000'  | '120'        | '3 900 000'  | '158 000 000' |
